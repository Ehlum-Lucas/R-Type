...
void collide_system(Registry &r)
{
    ...
    for (std::size_t i = 0; i < positions.size() && i < box_colliders.size() && i < sprites.size() && i < sizes.size() && i < ids.size() && i < onCollideDestroys.size(); ++i) {
        ...
        auto &size1 = sizes[i];
        auto &id1 = ids[i];
        auto &onCollideDestroy1 = onCollideDestroys[i];

        if (pos1 && box_collider1 && sprite1 && size1 && id1 && onCollideDestroy1) {
            ...
            for (std::size_t y = 0; y < positions.size() && y < box_colliders.size() && y < onCollideDestroys.size() && y < ids.size(); ++y) {
                ...
                auto &size2 = sizes[y];
                auto &id2 = ids[y];
                auto &onCollideDestroy2 = onCollideDestroys[y];

                if (pos2 && box_collider2 && id2 && sprite2 && size2 && y != i && onCollideDestroy2 && onCollideDestroy1.value().tag == box_collider2.value().tag) {
