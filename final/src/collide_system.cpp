...
void collide_system(Registry &r)
{
    ...
    auto &onCollides= r.get_components<OnCollide>();

    for (std::size_t i = 0; i < positions.size() && i < box_colliders.size() && i < sprites.size() && i < sizes.size() && i < ids.size() && i < onCollides.size(); ++i) {
        bool check = false;
        ...
        auto &size1 = sizes[i];
        auto &id1 = ids[i];
        auto &onCollide1 = onCollides[i];

        if (pos1 && box_collider1 && sprite1 && size1 && id1 && onCollide1) {
            ...
            col1.setFillColor(sf::Color(box_collider1.value().color.r, box_collider1.value().color.g, box_collider1.value().color.b));
            col1.setPosition(pos1.value().x, pos1.value().y);
            if (box_collider1.value().draw)
                r._window->draw(col1);
            for (std::size_t y = 0; y < positions.size() && y < box_colliders.size() && y < onCollides.size() && y < ids.size(); ++y) {
                auto &pos2 = positions[y];
                auto &box_collider2 = box_colliders[y];
                auto &sprite2 = sprites[y];
                auto &size2 = sizes[y];
                auto &id2 = ids[y];
                auto &onCollide2 = onCollides[y];

                if (pos2 && box_collider2 && id2 && sprite2 && size2 && y != i && onCollide2 && onCollide1.value().tag == box_collider2.value().tag) {
                    ...
                    std::cout << (onCollide1.value().type == "destroy" && onCollide2.value().type == "destroy") << std::endl;
                    if (col1.getGlobalBounds().intersects(col2.getGlobalBounds()) && onCollide1.value().type == "destroy" && onCollide2.value().type == "destroy") {
